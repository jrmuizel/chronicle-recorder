...
static void start_watching_maps(void)
{
  ...
  for (i = 0; i < n_starts; ++i) {
    NSegment* seg = VG_(am_find_nsegment)(starts_buf[i]);
...
static IRConst* add_trace_store(IRBB* bb, IRTemp tmp_trace_rec_ptr,
                                uint32_t offset, IRExpr* value)
{
  ...
  IRExpr* taddr = IRExpr_Binop(IR_ptr_add, IRExpr_Tmp(tmp_trace_rec_ptr),
                               offset_expr);
  addStmtToIRBB(bb, IRStmt_Tmp(tmp, taddr));
  addStmtToIRBB(bb, IRStmt_Store(IR_end, IRExpr_Tmp(tmp), value));
  return c;
}

static void add_trace_store_flatten(IRBB* bb, IRTemp tmp_trace_rec_ptr,
                                    uint32_t offset, IRExpr* value)
{
  IRTemp tmp = newIRTemp(bb->tyenv, typeOfIRExpr(bb->tyenv, value));
  addStmtToIRBB(bb, IRStmt_Tmp(tmp, value));
  add_trace_store(bb, tmp_trace_rec_ptr, offset, IRExpr_Tmp(tmp));
}

static void add_retirement_store(IRBB* bb, IRTemp tmp_retired_instructions_ptr, 
                                 uint8_t instr)
{
  IRExpr* e = IRExpr_Const(IRConst_U8(instr));
  addStmtToIRBB(bb, IRStmt_Store(IR_end,
                                 IRExpr_Tmp(tmp_retired_instructions_ptr),
                                 e));
}
...
static Int allocate_reg_effech_trace_offsets(IRBB* bb, Int offset, 
 ...
 */
static Bool find_base_temp(IRBB* bb, int i, IRExpr* expr, intptr_t* offset, IRTemp* tmp)
{
  intptr_t off;
  IRTemp last;
  ...
  tl_assert2(expr->tag == Iex_Tmp, "Non-flat expr %d", expr->tag);

  last = expr->Iex.Tmp.tmp;
  off = 0;
  while (i > 0) {
    --i;
    if (bb->stmts[i]) {
      IRStmt* st = bb->stmts[i];
      if (st->tag == Ist_Tmp && st->Ist.Tmp.tmp == last) {
        ...
        if (e->tag == Iex_Tmp) {
          ...
          intptr_t o;
          if (e1->tag == Iex_Tmp && e2->tag == Iex_Const) {
          ...
          } else if (e2->tag == Iex_Tmp && e1->tag == Iex_Const
...
static uint8_t get_log2_bytes(IRBB* bb, IRExpr* expr)
{
...
static IRExpr* add_trace_x87_indirech_reg(IRBB* bb, IRExpr* ix, Int bias)
{
  ...
  addStmtToIRBB(bb, IRStmt_Tmp(e1, IRExpr_Binop(Iop_Add32, ix, constBias)));
  addStmtToIRBB(bb, IRStmt_Tmp(e2, IRExpr_Binop(Iop_And32, IRExpr_Tmp(e1), const7)));
  addStmtToIRBB(bb, IRStmt_Tmp(e3, IRExpr_Binop(Iop_Add32, IRExpr_Tmp(e2), constRBase)));
  addStmtToIRBB(bb, IRStmt_Tmp(e4, IRExpr_Unop(Iop_32to8, IRExpr_Tmp(e3))));
  return IRExpr_Tmp(e4);
}
...
static IRBB* ch_instrument(VgCallbackClosure* closure, IRBB* bb_in, VexGuestLayout* layout,
                           VexGuestExtents* vge,
                           IRType gWordTy, IRType hWordTy )
{
  ...
  addStmtToIRBB(bb, IRStmt_Dirty(build_trace_record));

  addStmtToIRBB(bb,
                IRStmt_Tmp(tmp_retired_instructions_ptr,
                           IRExpr_Binop(IR_ptr_add, IRExpr_Tmp(tmp_trace_rec_ptr),
  ...
  for (i = 0; i < bb_in->stmts_used; ++i) {
      ...
      addStmtToIRBB(bb, st);
      {
        ...
        if (convert_offset_to_reg(st->Ist.Put.offset, &reg,
          ...
          if (update_SP) {
            ...
            addStmtToIRBB(bb, IRStmt_Dirty(update_SP_max));
          }
    ...
    case Ist_PutI:
      ...
      } else if (is_x87Tag_IRArray(st->Ist.PutI.descr)) {
      ...
      addStmtToIRBB(bb, st);
      {
        ...
        case Iex_Get: {
          ...
          if (convert_offset_to_reg(expr->Iex.Get.offset, &reg,
            ...
            if (offset_in_reg == 0) {
              reg_available_expressions[reg].temp = st->Ist.Tmp.tmp;
        ...
        case Iex_GetI:
          if (control_flags & CH_INITFLAG_LOG_REG_READS) {
            ...
            } else if (is_x87Tag_IRArray(expr->Iex.GetI.descr)) {
              /* do nothing, we (currently) don't care about tags */
            } else {
              ppIRBB(bb_in);
              tl_assert2(0, "Unknown array");
  ...
  if (!is_constant_address_expression(bb_in->next, next_instruction_addr) &&
    ...
    if (!stored_next_instruction_addr && bb_in->next->tag != Iex_Const) {
      ...
      IRExpr** args = mkIRExprVec_2(bb->next, IRExpr_Tmp(SP_temp));

      addStmtToIRBB(bb, IRStmt_Tmp(SP_temp, SP_expr));
